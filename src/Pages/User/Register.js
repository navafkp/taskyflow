...
const Register = () => {
    ...
    const handleRegistrationSubmit = async (e) => {
       ...
       console.log(formData)
        ...
        if (name, email, username, workspace, password, password2){
            try {
                await userSchema.validate(formData)
                const registrationResponse = await userRegister(name, username, email, workspace, password, password2)
    ...
    return (
        <div style={{ backgroundColor: '#B7A3F7' }}>
            <div className="h-screen flex flex-col items-center justify-center">
                ...
                <form className=" bg-black/30 shadow-md rounded px-8 pt-6 pb-8 mb-4 w-4/12 sm:w-11/12 md:w-8/12 lg:w-6/12 ">
                    <div className="mb-2">
                        <input required value={name} onChange={(e) => setName(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline  mt-2" id="name" type="text" placeholder="name" />
                    </div>
                    <div className="mb-2">
                    ...
                    <div className="mb-2">
                        <input value={workspace} onChange={(e) => setWorkapce(e.target.value)} className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline  mt-2" id="workspace" type="text" placeholder="workspace" />
                    </div>
                    <div className="mb-2">
                    ...
                    <div className="flex items-center justify-between">
                        <button onClick={handleRegistrationSubmit} className="bg-pink-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline " type="button" >
                            Regsiter
                        </button>
                        <p>Forgot Password?</p>
                    </div>
                </form>
                <p>Already member? <Link className='text-green-600 font-bold' to='/'>Login here</Link></p>
