...
import CreateCard from '../../Components/User/CreateCard'
import { useParams } from 'react-router-dom'
import { addingColumns, getAllColumns } from '../../Store/columnsSlice'
import { cardDragUpdate, cardsUpdate, getAllCards } from '../../Store/cardSlice'
import { dragCardUpdate } from '../../Server/User/getallCard'
  ...
  useEffect(() => {
    if (access && board && allcard?.cards?.length == 0) {
      ...
      dispatch(getAllColumns({ access, board_slug })).then((res) => {
        dispatch(getAllCards({ access, board_slug })).then((res) => {
